...
const Tabs = () => {
  ...
  return (
    ...
    <View style={[tw`py-[.7rem] px-8 rounded-t-[30px] bottom-0 absolute flex flex-row justify-between items-center w-full bg-white dark:bg-slate-700`, {zIndex: 0}]}>
      ...
      <Button onPress={()=> switchTo('SelectListToMap')}>
        <LocationDotSvg height={36} width={36} fill={theme == 'dark' ? ( selectedScreen == 'SelectListToMap' ? '#8B5CF6' : '#CBD5E1') : ( selectedScreen  == 'SelectListToMap' ? '#A78BFA' : '#D1D5DB') }/>
      </Button>
